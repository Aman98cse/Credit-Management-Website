...
<body>
	<header class="cd-main-header">
		...
		<br/><br/>
		<center><h2 style="font-size:25px;">Select the sender name by clicking on their name</h2></center><br/>
	<?php	
				...
				if($con){ 
					if($db){
						...
						while ($row=mysqli_fetch_assoc($res)) {
							echo '	<tr>
										<td>' . $row['id'] . '</td>	
										<td>'; ?><a href="user.php?id=<?php echo $row["id"]; ?>" ><?php echo $row["name"]; ?></a><?php echo '</td>						
										<td>' . $row['email'] . '</td>
										<td>' . $row['score'] . '</td>
									</tr>';	
						}
